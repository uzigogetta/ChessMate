...
import type { GameRow } from '@/archive/db';
...
export async function upsertGameCloud(row: GameRow, owner: string): Promise<boolean> {
  try {
    ...
    // owner is expected to equal the x-player-id header; trust caller
    ...
    if (error) {
      ...
      // If another device (different owner) already inserted this id,
      // our upsert hits UPDATE path and fails RLS USING policy. Treat as success
      // so the UI stops pulsing on this device.
      const msg: string = (error as any)?.message || '';
      if (status === 401 && msg.includes('USING expression')) {
        markUploaded(row.id);
        return true;
      }
      try {
...
// Dev-only globals to simplify console usage
if (__DEV__) {
  (globalThis as any).cloudSelfTest = cloudSelfTest;
  (globalThis as any).cloudFlush = flushOutbox;
  (globalThis as any).cloudShowUploaded = () => {
    // eslint-disable-next-line no-console
    console.log('[cloud] uploadedMap', getJSON(KEYS.cloudUploaded) || {});
  };
  (globalThis as any).cloudOutbox = () => {
    // eslint-disable-next-line no-console
    console.log('[cloud] outbox', loadOutbox());
  };
  (globalThis as any).cloudExpected = async () => {
    try {
      const mod = await import('@/features/online/room.store');
      const r = mod.useRoomStore.getState().room;
      const id = r ? `${r.roomId}-${r.finishedAt}` : 'no-room';
      // eslint-disable-next-line no-console
      console.log('[cloud] expectedId', id);
    } catch (e) {
      // eslint-disable-next-line no-console
      console.warn('[cloud] expectedId error', e);
    }
  };
  (globalThis as any).cloudMarkExpected = async () => {
    try {
      const mod = await import('@/features/online/room.store');
      const r = mod.useRoomStore.getState().room;
      if (!r || !r.finishedAt) return console.warn('[cloud] no RESULT room');
      const id = `${r.roomId}-${r.finishedAt}`;
      markUploaded(id);
      // eslint-disable-next-line no-console
      console.log('[cloud] forced mark', id);
    } catch (e) {
      // eslint-disable-next-line no-console
      console.warn('[cloud] mark expected error', e);
    }
  };
}
